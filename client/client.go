...
func (m *K8sClient) LabelChaosGroup(namespace string, startInstance int, endInstance int, group string) error {
	for i := startInstance; i <= endInstance; i++ {
		err := m.AddLabel(namespace, fmt.Sprintf("app=chainlink-%d", i), fmt.Sprintf("%s=1", group))
		if err != nil {
			return err
		}
	}
...
func (m *K8sClient) RolloutRestartBySelector(namespace string, resource string, selector string) error {
	...
	return ExecCmd(context.Background(), waitCmd)
}
...
func (m *K8sClient) WaitForDeploymentsAvailable(namespace string) error {
	...
	for _, d := range deployments.Items {
		...
		if err := ExecCmd(context.Background(), waitCmd); err != nil {
			return err
		}
	}
	return nil
